        ...
        <form action="{{ route('expenses.store') }}" method="post">
            ...
            <div class="mb-4">
                ...
                <x-tooltip message="Provide a description of the expense.">
                    <textarea name="description" id="description" cols="20" rows="3" class="input">{{ old('description') }}</textarea>
                </x-tooltip>
                @error('description')
                    <p class="error">{{ $message }}</p>
                @enderror
            ...
            <div class="flex justify-center items-center">
                <button type="submit" class="btn text-lg">Create Expense</button>
            </div>
        </form>
    </div>
</x-layout>
